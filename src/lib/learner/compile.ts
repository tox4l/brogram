...
const STATUS_RANK: Record<AccountStatus, number> = { active: 0, warned: 1, restricted: 2, banned: 3 }
...
/** Consecutive days ending on the most recent one with activity. */
function streakOf(timestamps: string[]): { days: number; last: string | null } {
 ...
 * `version + 1`. Points, streaks, the integrity score and the account status
 * are derived here, never trusted from the previous document.
 */
export function compileLearnerState(
  ...
  const computedStatus = statusFor(score, maxPasteInExercise(events, now))
  const storedStatus = profileRow.account_status ?? 'active'

  const exercise = streakOf(attemptRows.map((row) => row.created_at))
  const derot = streakOf((wellnessRow?.drill_results ?? []).map((result) => result.at))
  ...
  return {
    ...
    accountStatus: STATUS_RANK[computedStatus] > STATUS_RANK[storedStatus] ? computedStatus : storedStatus,
