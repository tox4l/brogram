...
beforeEach(() => {
  vi.stubGlobal('fetch', vi.fn((url: string) => {
    ...
    if (url === '/api/admin/bank-stats') {
      return jsonResponse({
        ok: true,
        stats: [{ clo_id: 'INFS1101-1', pattern: 'trace', seed_count: 2, generated_count: 0, verified_count: 0 }],
        clos: [{ id: 'INFS1101-1', ordinal: 1, course: 'INFS1101', patterns: ['trace'] }],
      })
    }
...
describe('/admin dashboard', () => {
  ...
  it('shows a retryable error for a section whose fetch fails', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      return jsonResponse({ ok: true, invites: [], users: [], stats: [], clos: [], usage: [] })
    }))
    const { AdminDashboard } = await import('./AdminDashboard')
    render(<AdminDashboard />)
