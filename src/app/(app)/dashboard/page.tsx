    ...
    async function load() {
      try {
        ...
        const [course, outcomes, exercises] = await Promise.all([
          ...
          courseCode ? client.from('clos').select('id,ordinal,outcome').eq('course', courseCode).order('ordinal') : { data: [], error: null },
...
function Statistic({ label, value, note }: { label: string; value: string; note: string }) {
  return (
    <div role="group" aria-label={label} className="min-w-0 py-1">
      ...
      <p className="mt-1.5 font-mono text-xl font-medium tracking-tight text-zinc-100">{value}</p>
  ...
  return (
    <div className="space-y-7">
      <div>
        ...
        <p className="mt-2 text-sm text-zinc-400">{learnerState?.currentCourse ? 'A little practice. A clearer understanding.' : 'Choose a course and make space for your first small win.'}</p>
      </div>

      <section id="course" aria-labelledby="course-heading" className="scroll-mt-6 rounded-xl border border-white/10 bg-linear-to-br from-emerald-200/[0.06] to-transparent p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <h2 id="course-heading" className="text-xs font-medium text-zinc-400">Current course</h2>
            ...
            <p className="mt-2 text-sm leading-relaxed text-zinc-400">{course ? `${languages[course.language] ?? course.language} · ${completed} of ${outcomes.length} outcomes complete` : learnerState?.currentCourse ? 'Your saved progress is kept below.' : 'A course gives your practice a direction. You can change it anytime.'}</p>
          </div>
          <Link href="/onboarding" className={cn(buttonVariants({ variant: learnerState?.currentCourse ? 'outline' : 'default' }), learnerState?.currentCourse ? 'h-9' : 'h-9 bg-emerald-200 text-zinc-950 hover:bg-emerald-100')}>
            {learnerState?.currentCourse ? 'Change course' : 'Choose a course'}<ArrowUpRight aria-hidden="true" />
          </Link>
        </div>
      </section>

      <div className="grid grid-cols-3 gap-3 border-b border-white/10 pb-5 sm:gap-6">
      ...
      {curriculum.loading && <p role="status" className="text-sm text-zinc-400">Loading your course and exercise details. Your saved progress is ready.</p>}
      {curriculum.failed && (
        <div role="alert" className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/15 p-4">
          <p className="text-sm text-zinc-300">Course and exercise details could not load. Your saved progress is still here.</p>
      ...
      <section aria-labelledby="next-heading">
        <div className="flex items-baseline justify-between gap-3">
          ...
          {orderedExercises.length > 0 && <p className="text-xs text-zinc-500">Your practice path</p>}
        </div>
        {restricted && <p className="mt-2 text-sm text-zinc-400">Exercises are paused while your account is restricted.</p>}
        {orderedExercises.length ? (
          <ol className="mt-3 divide-y divide-white/10 border-y border-white/10">
            {orderedExercises.map((exercise, index) => {
              const contents = <><span className="w-5 shrink-0 font-mono text-xs text-zinc-500">{String(index + 1).padStart(2, '0')}</span><span className="min-w-0 flex-1"><span className="block text-sm font-medium text-zinc-100">{exercise.title}</span><span className="mt-1 block text-xs text-zinc-400">{languages[exercise.language] ?? exercise.language} · Difficulty {exercise.difficulty} of 5</span></span>{restricted ? <LockKeyhole className="size-4 shrink-0 text-zinc-500" aria-hidden="true" /> : <ArrowRight className="size-4 shrink-0 text-emerald-200" aria-hidden="true" />}</>
              return <li key={exercise.id}>{restricted ? <div className="flex items-center gap-4 py-4">{contents}</div> : <Link href={`/exercise/${encodeURIComponent(exercise.id)}`} className="flex items-center gap-4 rounded-md py-4 outline-none transition-colors hover:bg-white/[0.03] focus-visible:ring-2 focus-visible:ring-emerald-300 motion-reduce:transition-none">{contents}</Link>}</li>
            })}
          </ol>
        ) : (
          <div className="mt-3 rounded-xl border border-dashed border-white/15 p-5">
            ...
            <p className="mt-1.5 max-w-lg text-sm leading-relaxed text-zinc-400">{exerciseIds.length ? 'Review your course to prepare a fresh practice path.' : 'Once your course and learning profile are ready, your next three exercises will appear here.'}</p>
        ...
        {orderedExercises.length > 0 && orderedExercises.length < exerciseIds.length && <p className="mt-3 text-sm text-zinc-400">Some recommendations are no longer available. Change your course to refresh your path.</p>}
      </section>

      <section aria-labelledby="mastery-heading">
        <div className="flex items-baseline justify-between gap-3">
          <h2 id="mastery-heading" className="text-base font-medium">Course mastery</h2>
          {outcomes.length > 0 && <p className="text-xs text-zinc-500">{completed} / {outcomes.length} complete</p>}
        </div>
        {outcomes.length ? (
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {outcomes.map((outcome) => {
              ...
              return (
                <div key={outcome.id} className="rounded-lg border border-white/10 p-4">
                  <div className="flex items-center justify-between gap-3 text-xs"><span className="text-zinc-500">Outcome {outcome.ordinal}</span><span className={mastery?.closed ? 'text-emerald-200' : 'text-zinc-400'}>{mastery?.closed ? 'Complete' : mastery ? 'In progress' : 'Not started'}</span></div>
                  <h3 className="mt-2 text-sm font-medium leading-relaxed">{outcome.outcome}</h3>
                  <div className="mt-4 flex items-center gap-3"><Progress value={score} aria-label={outcome.outcome} className="flex-1 [&_[data-slot=progress-indicator]]:bg-emerald-200" /><span className="font-mono text-xs text-zinc-400">{score}%</span></div>
                </div>
              )
            })}
          </div>
        ) : <div className="mt-3 rounded-xl border border-dashed border-white/15 p-5"><p className="text-sm font-medium">{curriculum.loading ? 'Opening your learning outcomes.' : curriculum.failed ? 'Your mastery details will return when reconnected.' : 'Your outcomes will appear here.'}</p><p className="mt-1.5 text-sm leading-relaxed text-zinc-400">Practice across different patterns to build confidence in each outcome.</p></div>}
      </section>

      <section aria-label="De-rot practice" className="flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-5">
        <div><h2 className="text-sm font-medium">A change of pace</h2><p className="mt-1 text-sm text-zinc-400">{derotDays > 0 ? 'Keep your attention streak going with a short drill.' : 'Train your attention with a short coding drill.'}</p></div>
