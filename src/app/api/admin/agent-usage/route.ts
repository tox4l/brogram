...
export async function GET() {
  ...
  try {
    ...
    const rows = await fetchAllRows<UsageRow>((from, to) =>
      svc
        .from('agent_usage')
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : 'unable to load agent usage' }, { status: 500 })
  }
}
