...
export function BuddyButton() {
  ...
  return (
    <Dialog>
      <DialogTrigger render={<Button variant="outline" className="h-9 border-input px-3 text-foreground" />}>
        <MessageCircle aria-hidden="true" />Buddy
      </DialogTrigger>
      <DialogContent className="top-0 right-0 left-auto h-dvh max-w-[min(24rem,100%)] translate-x-0 translate-y-0 content-start gap-8 rounded-none border-l border-border bg-background p-6 sm:max-w-sm">
        ...
        <div className="border-t border-border pt-5">
          <p className="text-sm text-muted-foreground">Buddy chat is not available yet. Your progress and practice links are ready below.</p>
          <div className="mt-4 flex flex-wrap gap-3">
            <Link href="/dashboard" className={buttonVariants({ variant: 'outline' })}>View progress</Link>
            <Link href="/derot" className={cn(buttonVariants(), 'bg-emerald-200 text-primary-foreground hover:bg-emerald-100')}>Try de-rot</Link>
          </div>
        </div>
      </DialogContent>
    </Dialog>
