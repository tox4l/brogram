...
export function AppShell({ children, wellnessRail, buddy }: {
  ...
  return (
    <div className="flex flex-1 flex-col">
      <a href="#main-content" className="sr-only z-50 rounded-md bg-emerald-200 px-4 py-3 text-zinc-950 focus:not-sr-only focus:fixed focus:top-3 focus:left-3">Skip to content</a>
          ...
          <nav aria-label="Main navigation" className="order-3 flex w-full items-center gap-1 sm:order-none sm:w-auto">
            {navigation.map((item) => (
              <Link key={item.title} href={item.href} aria-current={item.active ? 'page' : undefined}
                className={cn('rounded-md px-3 py-2 text-sm outline-none transition-colors hover:bg-white/5 hover:text-white focus-visible:ring-2 focus-visible:ring-emerald-300 motion-reduce:transition-none', item.active ? 'bg-white/5 text-white' : 'text-zinc-400')}>
                {item.title}
              </Link>
            ))}
          </nav>
      ...
      <div className={cn('mx-auto grid w-full max-w-7xl flex-1 gap-8 px-5 py-6 sm:px-8 lg:gap-10', !exercise && 'lg:grid-cols-[minmax(0,1fr)_15rem]')}>
        <main id="main-content" tabIndex={-1} className="min-w-0 outline-none">{children}</main>
        <aside aria-label="Wellness" className={cn('min-w-0 border-t border-white/10 pt-6', !exercise && 'lg:border-t-0 lg:border-l lg:pt-0 lg:pl-7')}>
