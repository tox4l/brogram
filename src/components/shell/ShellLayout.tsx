...
export function ShellLayout({ dock, children }: { dock: ReactNode; children: ReactNode }) {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'mx-auto flex w-full max-w-7xl flex-1 flex-col px-4 py-6 sm:px-8',
          template === 'top-strip' && 'gap-4',
          (template === 'left-rail' || template === 'right-rail') && 'gap-8 lg:grid lg:items-start lg:gap-12',
          template === 'left-rail' && 'lg:grid-cols-[17.5rem_minmax(0,1fr)]',
          template === 'right-rail' && 'lg:grid-cols-[minmax(0,1fr)_17.5rem]',
        )}
      >
        {template === 'top-strip' && (
          <aside aria-label="Wellness" className="w-full border-b border-rule pb-3">{dock}</aside>
        )}
