...
describe('AppShell', () => {
  ...
  // model resolves from `lg:` up, where the wordmark, nav and control
  // cluster actually fit on one line (measured live: the row needs ~704px
  // and does not fit until ~820px -- `sm:`/`md:` would clip it and force a
  // document-wide horizontal scrollbar, T4.5 fix round finding I1);
  // e2e/measure.spec.ts (T4.10) asserts the actual rendered geometry at
  // 1280x800 -- comfortably inside the `lg:` breakpoint.
  it('locks the header row to 56px (h-14) from the lg breakpoint up', () => {
    render(<AppShell>content</AppShell>)
    const wordmark = screen.getByRole('link', { name: /brogram/i })
    const headerRow = wordmark.parentElement
    expect(headerRow?.className).toMatch(/\blg:h-14\b/)
    expect(headerRow?.className).toMatch(/\blg:flex-nowrap\b/)
    expect(headerRow?.className).not.toMatch(/\bsm:flex-nowrap\b/)
  })
