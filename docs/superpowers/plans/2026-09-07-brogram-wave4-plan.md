...
Carry-overs from T4.0's third re-check (17:56 Doha, 2026-09-07): T4.5 takes N9 (the variant scan in contrast.test.ts anchors on the class token, so data-[state=open]:text-destructive and text-destructive/[0.7] in ui/badge.tsx and ui/button.tsx cannot bypass it) and N12 (THEMES entries carry a scheme field and DARK_IDS, THEME_TO_SONNER and the dark custom-variant list derive from it); T4.11 takes N10 (the budget test's CI claim), N11 (the fonts index comment about box-drawing glyphs) and N13 (the Fix round 3 section and the exact --unicodes recipe recorded in v2-T4.0-report.md).

Ruling (20:48 Doha, 2026-09-07), T4.4 step 7: the walkthrough prose column is capped at max-w-[34rem], not 68ch. Measured on f51a92b at 1280x800: ch is the advance of a zero, so 68ch is 811 px in Geist and 673 px in Newsreader at 18 px while the paragraph averages 8.2 and 7.4 px per character, which puts 68ch at roughly 99 and 91 rendered characters, and the shell affords the column only 680 px in the dark palettes so the cap never binds there; 34rem (544 px) measures 66 characters in Midnight and 73 in Folio, inside spec section 11 family C's 55 to 80 with the target near 68, and rem cannot drift with the face. The prose type (font-prose text-lede) returns to the paragraphs.

Ruling (21:58 Doha, 2026-09-07), the design allowlist: a screen pass deletes only the entries whose path prefixes fall inside its own ownership row, and only once those paths pass the rule with zero violations; an entry deleted for a path another task owns is a review failure and the deleting task's fix round restores it; T4.11 audits the final allowlist against the ownership map and the gate must be green at the wave close. The exercise workspace keeps a two-column step (brief beside code, results spanning below) at the default right-rail dock, where the shell affords 888 px, and the three-column layout applies from 75rem when the dock is off the rail; the brief's prose column is capped in rem like the walkthrough's.

Ruling (22:30 Doha, 2026-09-07), the exercise brief's measure: spec section 11 family C's 55 to 80 characters applies to reading columns (the walkthrough prose, the report, the course home copy). The exercise brief is a side panel beside a code editor with a 480 px floor inside the 888 px the default shell affords, so its band is 42 to 60 rendered characters at 16 px at both dock placements (measured 43 to 50 across the bank on 011ea85); T4.10's measure spec asserts that band for the brief selector and the family C band for the reading columns. Shrinking the 17.5rem dock rail to buy the brief 55 characters is refused; the rail's width is T4.5's design.

T4.11 carry-overs (01:26 Doha, 2026-09-08), all binding for the wave close: (1) the design gate exits 0: the primitives' half-step paddings and gaps (button, badge, tabs, input, drawer, the prayer marker) get one scoped allowlist entry with a dated owner beside the exact-count pin the sweep added; the three root-layout and Buddy-drawer lines are cleared by the second bundle lane's round 2; (2) the bundle gate exits 0 with no budget raised; if a route still needs a human ruling the review names the chunk, the module and the bytes; (3) T4.0's N10 (the budget test's CI claim), N11 (the fonts index comment about box-drawing glyphs) and N13 (the Fix round 3 section and the exact subset recipe recorded in the T4.0 report) are closed; (4) the proxy lane's two Minors are recorded for T3.6: the expired-restriction reconcile trusts the server clock over the database's, and a restricted banner can linger up to 45 s after a restriction ends; (5) the allowlist audit against the ownership map; (6) the build log carries the correction that T3.2's round 3 title overstated its change (the course transition was made fast by the proxy lane, not by streaming); (7) the ledger rows T4.0 to T4.11 read with their SHAs and the five-palette captures of every screen exist at 1280x800.

T4.11 carry-over 8 (01:54 Doha, 2026-09-08), the last two budget rows: after the bundle lanes, /login measures 388,668 B against 346,112 (the 231,786 B Supabase sign-in pair is an accepted row; the rest is the 54,609 B login page, three shared framework chunks and 16,891 B of next/dynamic glue for the account notice) and /admin measures 225,971 B against 203,776 (sonner 36,657 B, the invite UI 28,561 B, shared framework; no Supabase, react-query, gsap or zod). Neither carries a leak. Ruling: T4.11 first trims what is cheap on /login (the account notice mounts fully dynamically on the login route, and the login page chunk is audited for anything not needed before sign-in, the palette picker and shell imports first), re-measures, and then re-baselines the /login row to the measured worst of three rounded up to the next 10 KB with a ledger row naming the staged login; the /admin row is re-baselined to 230 KB with a ledger row naming the toasts and the invite UI as a staff-surface cost. Every other row stays; the bundle gate exits 0 at the wave close.
