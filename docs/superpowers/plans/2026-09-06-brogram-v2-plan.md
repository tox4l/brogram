...
- [ ] **Step 2: `e2e/perf.spec.ts`** — against a **production build** with `AGENT_DRY_RUN=true` so no model latency pollutes the numbers. Drives dashboard → course → lesson → exercise → submit, reads the marks and the navigation timing entries, runs each route **three times** and asserts the **median** against `perf-budget.json`. Round-trip counts are asserted by counting Supabase requests via `page.route` interception, per route:
  ...
  Interaction budgets asserted: course tile click → course home painted **< 150 ms warm / < 300 ms cold** (W2FIX-P paid the IOU T3.2 fix round 3 left here: `src/lib/supabase/middleware.ts`'s two serial network round trips — `rpc('lift_expired_restriction')` then a `profiles` select — now check a signed, httpOnly cache cookie first (`src/lib/supabase/profile-cache.ts`, TTL 60s) and skip both on a hit everywhere except `/exercise`, which always re-reads fresh so a restriction still bites immediately; the two calls run concurrently, not serially, when a fresh read is needed. Fix round F2 (Opus review): the 150ms number is a **cache-hit** number, true only within the 60s TTL of a preceding read on the route — it is not the transition cost for every click. Both paths are now measured and budgeted separately, three production-build runs each: warm (cache hit) 131.2ms, 87.7ms, 131.1ms, worst 131.2 rounded up to 150; cold (cache miss, cookie evicted immediately before the click) 248.1ms, 249.9ms, 250.3ms, worst 250.3 rounded up to 300 — the remaining cost being one concurrent `rpc` + `profiles` round-trip pair against the production Supabase project, roughly half the pre-fix serial cost); submit → pass/fail revealed **< 300 ms beyond runtime execution**; lesson check → verdict **< 120 ms** for non-`micro-code` kinds. Paint budgets: LCP ≤ 1.5 s on dashboard, course and lesson; ≤ 1.8 s on exercise; CLS ≤ 0.05 and INP ≤ 200 ms everywhere.
